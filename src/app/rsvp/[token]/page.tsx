...
export default async function RsvpPage({
  ...
  const suggestedStatus = query.response === "CONFIRMED" || query.response === "DECLINED"
    ? query.response
    : undefined;
  ...
  if (invitations.length === 0) {
    return <Unavailable message="This invitation link is invalid or is no longer available." />;
  }
  ...
  const recipientName = first.user?.name ?? first.externalName ?? first.user?.email ?? "Invitee";
      ...
      <div className="mx-auto max-w-3xl overflow-hidden rounded-[2rem] border border-[#d3deef] bg-card shadow-[0_24px_70px_rgba(0,53,128,0.08)]">
        ...
        <div className="space-y-7 px-6 py-8 sm:px-9">
          ...
          <dl className="grid gap-4 rounded-[1.5rem] border border-slate-200 bg-secondary/45 p-5 sm:grid-cols-2">
            ...
            <Detail icon={<UserRound className="h-4 w-4" />} label="Organizer" value={event.organizer.name ?? event.organizer.email} />
            {invitations.length > 1 ? (
              <Detail icon={<CalendarDays className="h-4 w-4" />} label="Series" value={`${invitations.length} occurrences; final meeting ${formatDate(finalEnd)}`} />
            ) : null}
          </dl>
          ...
          {closed ? (
            ...
            <RsvpResponseForm token={token} currentStatus={first.status} suggestedStatus={suggestedStatus} />
          )}

          <div className="flex gap-2 border-t border-slate-200 pt-5 text-xs leading-5 text-slate-500">
            <ShieldCheck className="mt-0.5 h-4 w-4 shrink-0" />
            This page is personal to your invitation. Do not share its address with another person.
          </div>
        </div>
      </div>
    </main>
  ...
  return (
    <div className="flex gap-3">
      ...
      <div>
        ...
        <dd className="mt-1 text-sm font-medium leading-5 text-slate-800">{value}</dd>
      </div>
    </div>
  );
}
