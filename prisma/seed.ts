...
async function main() {
  ...
  for (const u of MINISTRY_USERS) {
    ...
    await prisma.user.upsert({
      where: { email: u.email },
      update: { name: u.name, role: u.role, systemRole: u.systemRole, jobTitle: u.jobTitle, ministryId, passwordHash: defaultPasswordHash },
      create: {
        email: u.email,
        name: u.name,
        role: u.role,
  ...
  for (const u of SUPER_ADMIN_USERS) {
    await prisma.user.upsert({
      where: { email: u.email },
      update: { name: u.name, role: u.role, systemRole: u.systemRole, jobTitle: u.jobTitle, ministryId: null, passwordHash: superAdminPasswordHash },
      create: {
        email: u.email,
        name: u.name,
        role: u.role,
